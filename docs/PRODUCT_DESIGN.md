...
**Shipped (closed-loop adaptation):** The loop is now actually closed. Past patient-reported outcomes inform mission selection: if a patient reported a template as "harder" 2+ times, the system avoids re-offering it; if they reported "easier" and "noticed a difference," that template gets a small preference bonus (repetition with positive signal builds habit). Outcome-aware Mira flags on the operator surface surface struggle patterns ("Patient reported 2 missions as harder than expected this week") and positive signals ("Patient noticed a difference on 3 of 4 completed missions"). The clinician digest now includes a longitudinal outcome trend section showing the arc across all completed missions with reported outcomes.
...
- Closed-loop adaptation: past patient-reported outcomes inform mission selection (avoid templates reported "harder" 2+ times, prefer templates with positive signal). The loop is visible to the patient — Mira's opening line references past outcomes ("You said post meal walk felt easier and you noticed a difference"), the patient's free-form reflection ("You mentioned: 'I had more energy in the afternoon'"), and signals adaptation when offering a new mission ("Based on that, I'm suggesting something different today"). Milestone language marks the first completion ("That's your first one. Starting is the hardest part.") and the first noticed-difference ("That's the first time you've noticed a difference from one of these. That's the whole point."). Outcome-aware Mira flags (struggle + positive signal + barrier-outcome link) reference specific behaviours using structured PRO data. Longitudinal outcome trend in the clinician digest.
